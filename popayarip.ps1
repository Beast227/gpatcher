...
function Show-Usage {
@'
popayarip -- game patch producer/consumer
...
function Invoke-Verify {
    ...
    try {
        $manifestPath = $null
        if ($Against -like '*.zip') {
            $staging = New-TempDir 'popayarip-verify'
...
try {
    switch ($cmd) {
        ...
        'upload' {
            ...
            $desc    = Get-OptionalFlag $parsed.Flags 'description' 'Game patch generated by popayarip'
            Invoke-IAUpload `
