...
def invoke_interactive_menu():
    ...
    while running:
        ...
        elif selection == 4:  # Upload Patch to Internet Archive
            draw_box_top("Upload Patch to Internet Archive")
            patch = read_path_input("Enter local patch ZIP path", must_exist=True)
            if patch is None: continue
            creator = read_text_input("Enter creator name", default="anonymous")
            if creator is None: continue
            desc = read_text_input("Enter description", default="Game patch generated by gpatcher")
            if desc is None: continue
            draw_box_bottom()

            print(f"\n  {Colors.YELLOW}> Uploading patch...{Colors.RESET}")
            try:
                invoke_upload(patch, creator=creator, description=desc)
            except Exception as e:
                log_err(f"Upload failed: {e}")

        elif selection == 5:  # Verify Installation
        ...
        elif selection == 6:  # Diagnostics (Doctor)
